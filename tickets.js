...
function calculateCharges(ticketCount, category, selectedDurations) {
  const charges = {
  ...
  selectedDurations.forEach(duration => {
    const [startHour] = duration.split(' - ');
    const isPeak = (startHour >= '10.00 am' && startHour <= '12.00 pm') || (startHour >= '3.00 pm' && startHour <= '5.00 pm');
    const chargePerHour = isPeak ? charges[category].peak : charges[category].normal;
    totalCharge += chargePerHour * ticketCount;
  });
  ...
  function updateSummaryTable() {
    ...
    for (const category in selectedGuests) {
      if (selectedGuests[category] > 0) {
        ...
        const selectedDurationCount = selectedDurations.length;
        const totalCharge = calculateCharges(ticketCount, category, selectedDurations);
